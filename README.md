...
scripts/
  backup_profile.py  archives the Chrome profile and the database (unreproducible state)
...
Two things in this app cannot be regenerated by rerunning something, and
`scripts.backup_profile` archives both into a single zip:

- `secrets/ebay_chrome_profile/` — recreating it means an interactive eBay
  sign-in through `scratch_ebay_browser_login.py`, with whatever 2FA or
  captcha eBay serves that day.
- `scraperagent.db` (`config.DB_PATH`) — negotiation transcripts, seller
  replies, offer history, and per-search cost records. The file itself
  recreates on boot with an empty schema, but its contents do not; they're
  gitignored, and losing them means losing them for good.

Everything else rebuilds itself: tokens refetch, listings re-search.

```
python -m scripts.backup_profile              # defaults from config
python -m scripts.backup_profile --keep 10
python -m scripts.backup_profile --db-path D:/other/scraperagent.db
```
...
so a scheduled backup works while the browser is open. A missing or
zero-length database is likewise just a warning, not a failure — a fresh
install legitimately has neither yet.
